...
/**
 * 	File contentente la classe ServiceReportTest
 * 
 *	@file		ServiceReportTest.java
 *	@author		DeSQ
 *	@date		2014-05-14
 *	@version	1.0
 */
...
/**
 *	Classe contenente tutti i test di unità dei metodi della classe ServiceReport.
 *
 *	@author 	DeSQ
 */
public class ServiceReportTest {
	...
	/**
	 * 	Test del metodo serviceOperation() che permette di ottenere un report con le statistiche di un determinato processo.
	 * @throws Exception segnala un problema di connessione al database
	 */
	@Test
	public void testServiceOperation() throws Exception {
		ServiceReport serviceReportObject = new ServiceReport();
		...
		JSONObject jsonObject = serviceReportObject.serviceOperation(iEntityList);
	
		assertEquals(testProcess.getName(), jsonObject.get("Process"));
		assertEquals(1, jsonObject.get("AllSubscription"));
		assertEquals(1, jsonObject.get("SubscriptionComplete"));
		assertEquals(0, jsonObject.get("SubscriptionNotComplete"));
	}
