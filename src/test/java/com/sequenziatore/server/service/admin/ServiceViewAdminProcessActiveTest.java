...
/**
 * 	File contentente la classe ServiceViewAdminProcessActiveTest
 * 
 *	@file		ServiceViewAdminProcessActiveTest.java
 *	@author		DeSQ
 *	@date		2014-05-14
 *	@version	1.0
 */
...
/**
 *	Classe contenente tutti i test di unità dei metodi della classe ServiceViewAdminProcessActive.
 *
 *	@author 	DeSQ
 */
public class ServiceViewAdminProcessActiveTest {
	...
	/**
	 *	Test del metodo serviceOperation() che permette di visualizzare la lista di tutti i processi attivi amministrati da un dato amministratore.
	 * @throws Exception segnala un problema di connessione al database
	 */
	@Test
    public void testServiceOperation() throws Exception {
    	ServiceViewAdminProcessActive serviceViewAdminProcessActiveObject = new ServiceViewAdminProcessActive();
	    ...
	    JSONObject jsonReturnedObject = serviceViewAdminProcessActiveObject.serviceOperation(iEntityList);
    	
	    //estrazione dati dall'oggetto JSON
	    JSONArray jsonReturnedArray = (JSONArray) jsonReturnedObject.get("ListProcess");
	    JSONObject processData = (JSONObject) jsonReturnedArray.get(0);
	    
    	assertEquals(testProcess.getIdProcess(), processData.get("IdProcess"));
    	...
    	adminUser.setUsername("testAdmin1");
    	adminUser.setPassword("password");
    	HibernateUtil.getSession().saveOrUpdate(adminUser);
    	HibernateUtil.commitTransaction();
    	
    	iEntityList.remove(0);
    	iEntityList.add(adminUser);
    	
    	jsonReturnedObject = serviceViewAdminProcessActiveObject.serviceOperation(iEntityList);
    	
    	assertEquals("noneActiveProcess", jsonReturnedObject.get("Confirmation"));
	}
