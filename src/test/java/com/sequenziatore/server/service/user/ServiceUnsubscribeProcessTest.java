...
/**
 * 	File contentente la classe ServiceUnsubscribeProcessTest
 * 
 *	@file		ServiceUnsubscribeProcessTest.java
 *	@author		DeSQ
 *	@date		2014-05-14
 *	@version	1.0
 */
...
/**
 *	Classe contenente tutti i test di unità dei metodi della classe ServiceUnsubscribeProcess.
 *
 *	@author 	DeSQ
 */
public class ServiceUnsubscribeProcessTest {
	...
	public void testServiceOperation() throws Exception {
		ServiceUnsubscribeProcess serviceUnsuscribeProcessObject = new ServiceUnsubscribeProcess();
	    ...
	    JSONObject jsonObject = serviceUnsuscribeProcessObject.serviceOperation(iEntityList);
	    
	    HibernateUtil.getSession().beginTransaction();
	    assertNull(new DAOFactory().createDAOManagementProcessUser().findDataCollection(testUser, testStep));
	    assertEquals(1, new DAOFactory().createDAOManagementProcessUser().findAvailableProcess(testUser).size());
	    HibernateUtil.commitTransaction();
	    
	    assertEquals("successUnsubscribe", jsonObject.get("Confirmation"));
	}
